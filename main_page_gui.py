...
class LeftFrame(tk.Frame):
    def __init__(self, parent):
        ...
        # buttons
        self.b_all = tk.Button(self.left_lower, text='All', fg="black", width=25, height=2, bg='#dadada', command=gc.filter_platform('all', parent.img_ls))
        self.b_ins = tk.Button(self.left_lower, text='Instagram', fg="black", width=25, height=2, bg='#dadada', command=gc.filter_platform('instagram', parent.img_ls))
        self.b_pin = tk.Button(self.left_lower, text='Pinterest', fg="black", width=25, height=2, bg='#dadada', command=gc.filter_platform('pinterest', parent.img_ls))
        self.b_link = tk.Button(self.left_lower, text='LinkedIn', fg="black", width=25, height=2, bg='#dadada', command=gc.filter_platform('linkedIn', parent.img_ls))
        self.b_whi = tk.Button(self.left_lower, text='We Heart It', fg="black", width=25, height=2, bg='#dadada', command=gc.filter_platform('weheartit', parent.img_ls))
        
        self.b_graph_ins = tk.Button(self.left_lower, text='Instagram\n Like Graph', fg="white", width=10, height=4, bg='#9fbac1', command=gc.display_likes('instagram'))
        self.b_graph_whi = tk.Button(self.left_lower, text='We Heart It\n Like Graph', fg="White", width=10, height=4, bg='#9fbac1', command=gc.display_likes('weheartit'))
    ...
    def __init__(self, parent):
        ...
        for ind in df.index:
            ...
            try:
                ...
                tk.Button(self.inner_frame, image=parent.img_ls[ind], text='img: ' + str(ind), width=425, height=300, bg='grey', fg='white').grid(column=0, row=ind, pady=8, padx=50)
                tk.Label(self.inner_frame, text='@'+df_shuffled['User'][ind],fg='white',bg='grey').grid(column=0, row=ind, pady=8, padx=50,sticky='NW')
                tk.Label(self.inner_frame, text='❤ '+df_shuffled['Likes'][ind],fg='red',bg='grey').grid(column=0, row=ind, pady=8, padx=50,sticky='SE')
                img.close()
            except:
                print('failed')
                break
